...
class Regions:
    ...
    def avalanches(self,regs=None,states=None,thresh=30,window=0.05,step=1,smooth=None, profile=False):
        ...
        for i, r in enumerate(regs):
            if profile:
                size[r], intervals[r], size_t[r], profiles[r] = regions.computation.avalanchesFromProfile(fr[:,i+1],thresh,time_step=fr[1,0]-fr[0,0],t0=fr[0,0], profile=profile)
            else:
                size[r], intervals[r], size_t[r] = regions.computation.avalanchesFromProfile(fr[:,i+1],thresh,time_step=fr[1,0]-fr[0,0],t0=fr[0,0])

        if profile:
            return size, intervals, size_t, profiles
